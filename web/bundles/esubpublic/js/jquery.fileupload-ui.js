    ...
    $.widget('blueimpUI.fileupload', $.blueimp.fileupload, {
        ...
        options: {
            ...
            // Callback for successful uploads:
				...
				$('.fileupload-buttonbar.delete_all').show();
            },
        ...
        _deleteHandler: function (e) {
			...
			if((filesList.children('.template-download').length - 1) <= 0){
				$('.fileupload-buttonbar.delete_all').hide();
			}
        },
        ...
        _initButtonBarEventHandlers: function () {
            ...
            fileUploadButtonBar.find('.delete')
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
                    filesList.find('.name input:checked').parent('td').parent('tr').find('.delete')
                        .find('button').click();
                         fileUploadButtonBar.find('.cancel').trigger('click');
					if((filesList.children('.template-download').length - filesList.find('.name input:checked').length) <= 0){
						$('.fileupload-buttonbar.delete_all').hide();
					}	
                });
            fileUploadButtonBar.find('.download')
                .bind('click.' + ns, function (e) {
                    e.preventDefault();
                    var clean_box = $(this).parent('div').parent('div').parent('form').parent('div');
					var overlay = clean_box.find('.white_overlay');
					var iframe;
					overlay.css({
						width : clean_box.width()+1,
						height : clean_box.height()-40
					});
					overlay.find('img').css({
						marginTop : (clean_box.height()-40)/2-15
					});
					overlay.show();
					iframe = $('<iframe>');
					iframe.css({
					...
					iframe.attr('src', 'clean/download');
					clean_box.append(iframe);
                                        setTimeout(function(){
                                                    //iframe.remove();
                                                    overlay.hide();
                                            }, 2000);
            ...
            fileUploadButtonBar.find('.toggle')
                .bind('change.' + ns, function (e) {
                    filesList.find('.name input').prop(
                        'checked',
                        $(this).is(':checked')
                    );
                });
